...
public abstract class ALevelDefinition implements ILevelDefinition {
    ...
    public ALevelDefinition(
            Class<? extends Annotation> annotationClassFieldLevel,
            Class<? extends Annotation> annotationClassParameterLevel,
            Class<? extends Annotation> annotationClassReturnLevel,
            Class<? extends Annotation> annotationClassEffects,
    ...
    public abstract Set<LEQConstraint> extractConstraints(IAnnotationDAO dao,
                                                          String signature);
        ...
        if (!returnType.equals("void")) {
            for (int i = 0; i < parameterTypes.size(); i++) {
                ComponentParameterRef paramRef =
                    new ComponentParameterRef(i, signature);
                constraints.add(new LEQConstraint(paramRef, returnRef));
            }
            if (parameterTypes.size() == 0)
                constraints.add(new LEQConstraint(getGreatesLowerBoundLevel(),
                                                  returnRef));
        }
        return constraints;
    }
