...
public interface LevelEquationVisitor {
	...
	public static class LevelEquationCalculateVoidVisitor implements LevelEquationVisitor {
		...
		public boolean isValid() {
			return containedLevels.size() == 1 && containedLevels.contains(LevelMediator.VOID_LEVEL);
		}
	...
	public static class LevelEquationEvaluationVisitor implements LevelEquationVisitor {
		...
		/** */
		private LevelMediator mediator = null;
		/** */
		...
		protected LevelEquationEvaluationVisitor(List<String> argumentLevels,
				List<String> parameterLevels, LevelMediator mediator) {
			...
			if (parameterLevels.size() == argumentLevels.size()) {
				for (int i = 0; i < parameterLevels.size(); i++) {
					...
					if (parameterLevel.startsWith(LevelMediator.LEVEL_PATTERN_SIGN)) {
		...
		public void visit(Level level) {
			...
			if (lev.startsWith(LevelMediator.LEVEL_PATTERN_SIGN)) {
		...
		public void visit(LevelMaxEquation levelEquation) {
			if (resultingLevel.size() == 2) {
				try {
					String result = mediator.getMaxLevel(resultingLevel.get(0), resultingLevel.get(1));
		...
		public void visit(LevelMinEquation levelEquation) {
			if (resultingLevel.size() == 2) {
				try {
					String result = mediator.getMinLevel(resultingLevel.get(0), resultingLevel.get(1));
					resultingLevel.clear();
					resultingLevel.add(result);
				} catch (Exception e) {
	...
	public static class LevelEquationValidityVistitor implements LevelEquationVisitor {
		 ...
		 * @param mediator
		 */
		protected LevelEquationValidityVistitor(LevelEquation levelEquation, LevelMediator mediator) {
			this.mediator = mediator;
		...
		public void visit(Level level) {
			containedLevels.add(level.getLevel());
			boolean result = mediator.getAvailableLevels().contains(level.getLevel()) || level.getLevel().equals(LevelMediator.VOID_LEVEL);
			if (! result) {
				valid = false;
			} else {
