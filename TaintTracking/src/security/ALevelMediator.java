...
public abstract class ALevelMediator implements ILevelMediator {
    ...
    public final Set<LEQConstraint> extractConstraints(SootClass sootClass) {
        try {
            ...
            return definition.extractConstraints(dao, sootClass.getName());
                ...
                | AnnotationElementNotFoundException
                | AnnotationInvalidConstraintsException e) {
            throw new AnnotationExtractionException(getMsg("exception.annotation.extract_class_constraints_error",
                                                           getSignatureOfClass(sootClass)),
                                                    e);
        }
    }
    ...
    public ILevel getGreatestLowerBoundLevelOf(List<ILevel> levels) {
        if (levels.size() > 0) {
            ...
            for (int i = 1; i < levels.size(); i++) {
                result =
                    this.definition.getGreatestLowerBoundLevel(result,
                                                               levels.get(i));
            }
            return result;
        }
    ...
    public ILevel getLeastUpperBoundLevelOf(List<ILevel> levels) {
        if (levels.size() > 0) {
            ...
            for (int i = 1; i < levels.size(); i++) {
                result =
                    this.definition.getLeastUpperBoundLevel(result,
                                                            levels.get(i));
            }
            return result;
        }
    ...
    public Set<LEQConstraint> getLibraryConstraints(SootMethod sootMethod) {
        ...
        return this.definition.getLibraryConstraints(sootMethod.getName(),
                                                     paramType,
                                                     returnType,
    ...
    public List<ILevel> getLibraryFieldSecurityLevel(SootField sootField) {
        // FIXME: ARRAY
        return this.definition.getLibraryFieldLevel(sootField.getName(),
    ...
    public List<ILevel> getLibraryParameterSecurityLevel(SootMethod sootMethod) {
        ...
        return this.definition.getLibraryParameterLevel(sootMethod.getName(),
                                                        paramType,
                                                        sootMethod.getDeclaringClass()
                                                                  .getName(),
                                                        sootMethod.getSignature());
    ...
    public ILevel getLibraryReturnSecurityLevel(SootMethod sootMethod,
                                                List<ILevel> levels) {
        List<String> paramType = new ArrayList<String>();
        for (Object type : sootMethod.getParameterTypes()) {
            paramType.add(type.toString());
        }
        return this.definition.getLibraryReturnLevel(sootMethod.getName(),
                                                     paramType,
                                                     sootMethod.getDeclaringClass()
                                                               .getName(),
                                                     sootMethod.getSignature(),
    ...
    public List<ILevel> getLibraryWriteEffects(SootMethod sootMethod) {
        ...
        return this.definition.getLibraryMethodWriteEffects(sootMethod.getName(),
                                                            paramType,
                                                            sootMethod.getDeclaringClass()
                                                                      .getName(),
                                                            sootMethod.getSignature());
    ...
    public final boolean hasClassWriteEffectAnnotation(SootClass sootClass) {
        return hasAnnotationWithType(sootClass,
                                     getJNISignature(this.definition.getAnnotationClassEffects()));
    }

    public final boolean hasConstraintsAnnotation(SootClass sootClass) {
        return hasAnnotationWithType(sootClass,
                                     getJNISignature(this.definition.getAnnotationClassConstraints()));
    }
    ...
    public final boolean hasFieldSecurityAnnotation(SootField sootField) {
        return hasAnnotationWithType(sootField,
                                     getJNISignature(this.definition.getAnnotationClassFieldLevel()));
    }
    ...
    public final boolean hasParameterSecurityAnnotation(SootMethod sootMethod) {
        return hasAnnotationWithType(sootMethod,
                                     getJNISignature(this.definition.getAnnotationClassParameterLevel()));
    }

    public final boolean hasReturnSecurityAnnotation(SootMethod sootMethod) {
        return hasAnnotationWithType(sootMethod,
                                     getJNISignature(this.definition.getAnnotationClassReturnLevel()));
    }
    ...
    private final boolean hasAnnotationWithType(Host host, String type) {
        try {
            if (hasVisibilityAnnnotationTag(host)) {
                ...
                if (hasAnnotationOfType(tag, type))
                    return true;
            }
            return false;
