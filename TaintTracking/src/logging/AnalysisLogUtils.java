 ...
 * 
 * The {@link AnalysisLogUtils} provides multiple methods and constants which
 * are mainly required by the {@link SootLogger} and its subclasses as well as
 * the formatter {@link SootLoggerFileFormatter} and
 * {@link AnalysisLogConsoleFormatter}. E.g. some methods allow to generate
 * different {@link Handler} which are already adjusted and which will output
 * the messages on different ways. Other methods allow the conversion of certain
 * objects to Strings, or they simplifying the generation of required Strings.
 * 
 * <hr />
 * 
 * @author Thomas Vogel
...
public class AnalysisLogUtils {
    ...
    /**
     * Name of the setting which indicates whether the log should be stored as a
     * text file.
     */
    public static final String TXT_EXPORT_FILE = "Export File";
    /**
     * Name of the setting which indicates whether the log should be printed
     * during the analysis.
     */
    ...
    /**
     * {@link Formatter} which formats the messages for outputting them as text
     * files.
     */
    private static final AnalysisLogConsoleFormatter FORMATTER_CONSOLE =
        new AnalysisLogConsoleFormatter();
    ...
    protected static final String TXT_LINE_SEPARATOR =
    ...
    /**
     * Generates a String from the given {@link Settings}. Each single
     * {@link Setting} of the settings is separated by a newline character.
     * 
     * @param settings
     *            Settings for which a String should be generated that contains
     *            the information of this settings.
    ...
    public static String generateSettingsString(Settings settings) {
        ...
        for (String name : settingsMap.keySet()) {
            result +=
                (result.equals("") ? "" : TXT_LINE_SEPARATOR) + name + ": "
    ...
    /**
     * Generates the {@link Setting}, which indicates whether the logged
     * messages should be printed instantaneously by the logger.
     * 
     * @param instantLogging
     *            {@code true} if the messages should be printed directly,
     *            otherwise {@code false}.
     * @return Setting of the instant logging.
     */
    public static Setting makeInstantLoggingSetting(boolean instantLogging) {
        return new Setting(TXT_INSTANT_LOGGING, (instantLogging ? TXT_ON
                                                               : TXT_OFF));
    }

    /**
     * Generates the {@link Setting}, which lists all the levels which are
     * enabled for the logging.
     * 
     * @param logLevels
    ...
    /**
     * Generates the {@link Setting}, which represents the start time of the
     * analysis.
     * 
    ...
    /**
     * Returns a String which contains the given String the given number of
     * times. E.g. {@code repeat("a", 3).equals("aaa")},
     * {@code repeat("a", 0).equals("")} and
     * {@code repeat("Ab", 3).equals("AbAbAb")}.
     * 
     * @param str
     *            String which should be repeated the given number of times.
     * @param n
     *            Number that indicates how many times the given String should
     *            be repeated.
    ...
    /**
     * Checks whether the given level is enable for the logging, i.e. it is
     * contained by the given array that contains those level which are enabled.
     * If this array contains {@link Level#ALL}, then the method will return for
     * every given level {@code true}. If the the array contains
     * {@link Level#OFF} then the method will return always {@code false}.
     * 
     * @param level
     *            Level for which should be checked whether the logging for this
     *            level is enabled.
     * @param levels
     *            Array of levels that contains the levels which are enabled.
     * @return {@code true} if the given array contains {@link Level#ALL},
     *         {@code false} if the array contains {@link Level#OFF}. Otherwise,
     *         if both are not contained by the given array, the method will
     *         return {@code true} if the given message is contained by the
     ...
     * 
     * @param levels
     *            Array of levels for which should be checked whether it
     *            contains the given level.
     * @param lookup
     *            {@link Level} for which should be checked whether it is
     *            contained by the given level array.
     * @return {@code true} if the given array contains the given level,
     *         otherwise {@code false}.
     */
    private static boolean containsArraySpecificLevel(Level[] levels,
                                                      Level lookup) {
        for (Level level : levels) {
    ...
    /**
     * Generates a formatted String which contains a default message for which
     * the message level has no applicable formatting. The message string
     * contains the level as well as the given level. Note: this method is
     * provided for the {@link Formatter} subclasses.
     * 
     * @param levelName
     *            {@link AnalysisLogLevel} name, which possesses the given
     *            message.
     * @param msg
     *            Message which should be formatted for printing.
     * @return A formatted default message which contains the given level as
    ...
    /**
     * Generates a formatted String which represents heading of a message. The
     * given level represents the level of the logged message and the array of
     * objects may contain additional information which should be shown in the
     * heading. The most messages have multiple tabs in front of the heading.
     * The number of the tabs can be given by a {@link HeadingInformation}
     * instance, or by a {@link ExtendedHeadingInformation} instance which also
     * provides informations about the file name and the source line number
     * where the message was generated. Note: this method is provided for the
     * {@link Formatter} subclasses.
     * 
     * @param levelName
     *            {@link AnalysisLogLevel} name of the message for which the
     *            heading will be generated.
     * @param parameters
     *            Array of objects, which may contain a
     *            {@link HeadingInformation} or a
     *            {@link ExtendedHeadingInformation} for providing informations
     *            about the number of tabs in front of the heading as well as
     *            the file name and the source line number where the message was
     *            generated.
     * @return A formatted String which represents heading of a message. This
     *         contains the given level and if available, the file name and the
     *         source line number.
     * @see SootLoggerFileFormatter
     * @see AnalysisLogConsoleFormatter
     */
    protected static String generateLogHeading(String levelName,
                                               Object[] parameters) {
        StringBuilder result = new StringBuilder();
        String prefix = "";
        ...
        if (parameters != null && parameters.length == 1) {
            ...
            if (obj instanceof ExtendedHeadingInformation) {
                ExtendedHeadingInformation info =
                    (ExtendedHeadingInformation) obj;
                prefix = repeat(TXT_TAB, info.getTabs());
                additionalInfo =
                    " (" + info.getFileName() + FILE_SUFFIX_JAVA + ":"
                            + info.getSrcLn() + ")";
        ...
        result.append(prefix + TXT_OPEN_TAG + levelName + additionalInfo
                      + TXT_CLOSE_TAG + TXT_LINE_SEPARATOR);
        return result.toString();
    }

    /**
     * Generates a formatted String which contains a structural information. The
     * message is framed by predefined tags. Note: this method is provided for
     * the {@link Formatter} subclasses.
     * 
     * @param msg
    ...
    /**
     * Checks whether the given level is a {@link AnalysisLogLevel} that can be
     * handled as a standard level by the formatter. These standard levels
     ...
     * 
     * @param level
     *            Level for which should be checked whether it is a standard
     *            {@link AnalysisLogLevel} .
     * @return {@code true} if the given level is one of the standard level,
     *         {@code false} otherwise.
     * @see AnalysisLogConsoleFormatter
     * @see SootLoggerFileFormatter
     */
    protected static boolean isStandardLoggableMessage(Level level) {
        return level.equals(WARNING) || level.equals(SIDEEFFECT)
               || level.equals(SECURITY) || level.equals(CONFIGURATION);
