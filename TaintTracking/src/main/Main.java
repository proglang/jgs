...
import static util.AnalysisUtils.getArgumentParser;
import static util.DefinitionClassHandler.getDefinitionClass;
...
import util.AnalysisUtils.ArgumentParser;
 ...
 * the {@link SecurityLevelAnalysis} analysis to check for violations against
 * the <em>security level</em> and <em>write effect</em> policy.
 * 
 * <hr />
 * 
 * @author Thomas Vogel
...
public class Main {
    ...
    private static AnalysisLog execute(String[] args) {
        ...
        AnalysisLog log =
            new AnalysisLog(parser.isInstantLogging(), parser.getLogLevels());
        log.configuration(new Settings(makeLoggerLevelSetting(parser.getLogLevels()),
                                       makeInstantLoggingSetting(parser.isInstantLogging()),
                                       makeTimeSetting()));
        ILevelDefinition definition =
            getDefinitionClass(parser.getDefinitionClassPath());
        @SuppressWarnings("unused")
        ILevelDefinitionChecker checker =
            new DefinitionChecker(definition, log, true);
        ILevelMediator mediator = new Mediator(definition);
        AnalysisType type = parser.getAnalysisType();
        AnnotationExtractor extractor =
            new AnnotationExtractor(log, mediator, type);
        PackManager.v()
                   .getPack("wjtp")
        ...
        PackManager.v()
                   ...
                   .add(new Transform(PHASE_NAME_SECURITY,
                                      new SecurityTransformer(mediator,
                                                              log,
                                                              extractor,
                                                              parser.isInstantLogging(),
                                                              type)));
        soot.Main.main(args);
