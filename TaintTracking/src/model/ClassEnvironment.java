...
public class ClassEnvironment extends Environment {
     ...
     * @param mediator
     */
    public ClassEnvironment(SootClass sootClass,
        ...
        if (type.equals(LEVELS)) {
            if (!getLevelMediator().checkLevelsValidity(writeEffects)) {
                ...
                for (ILevel invalidEffect : getLevelMediator().getInvalidLevels(writeEffects)) {
                    throw new AnnotationInvalidException(getMsg("exception.effects.class_invalid",
            ...
            if (!containsSetProgramCounterReference(constraints)) {
                throw new AnnotationInvalidException(getMsg("exception.constraints.class_no_pc_ref",
                                                            classSignature));
            }
            if (containsSetParameterReference(constraints)) {
                throw new AnnotationInvalidException(getMsg("exception.constraints.param_ref",
                                                            classSignature));
            }
            if (containsSetReturnReference(constraints)) {
                throw new AnnotationInvalidException(getMsg("exception.constraints.return_ref",
                                                            classSignature));
            }
            ...
            if (!getLevelMediator().checkLevelsValidity(containedLevels)) {
                for (ILevel invalidEffect : getLevelMediator().getInvalidLevels(containedLevels)) {
                    throw new LevelInvalidException(getMsg("exception.constraints.class_invalid_level",
                                                           invalidEffect.getName(),
                                                           classSignature));
                }
            }
