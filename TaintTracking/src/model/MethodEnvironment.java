...
public class MethodEnvironment extends Environment {
     ...
     */
    private Map<Integer, MethodParameter> methodParameters =
        new HashMap<Integer, MethodParameter>();
    /**
    ...
    public MethodEnvironment(SootMethod sootMethod, boolean isIdFunction,
            ...
            ILevel returnSecurityLevel, List<ILevel> methodWriteEffects,
            List<ILevel> classWriteEffects, Set<LEQConstraint> constraints,
            AnalysisLog log, ILevelMediator mediator) {
        super(log, mediator);
    ...
    public List<MethodParameter> getMethodParameters() {
        List<MethodParameter> methodParametersList =
    ...
    public void isReasonable(AnalysisType type) {
        ...
        if (type.equals(CONSTRAINTS)) {
            ...
            for (IComponent returnRef : getInvalidReturnReferencesOfSet(constraints,
                                                                        signature,
                                                                        getDimension(sootMethod.getReturnType()))) {
                throw new AnnotationInvalidException(getMsg("exception.constraints.invalid_return_ref",
                                                            methodSignature,
                                                            returnRef.toString()));
            }
            if (isVoid) {
                if (containsSetReturnReferenceFor(constraints, signature)) {
                    throw new AnnotationInvalidException(getMsg("exception.constraints.void_method",
                                                                methodSignature));
                }
            } else {
                if (!containsSetReturnReferenceFor(constraints, signature)) {
                    throw new AnnotationInvalidException(getMsg("exception.constraints.no_return_ref",
                                                                methodSignature));
                }
            }
            List<ILevel> containedLevels =
                new ArrayList<ILevel>(getContainedLevelsOfSet(constraints));
            if (!getLevelMediator().checkLevelsValidity(containedLevels)) {
                for (ILevel invalidEffect : getLevelMediator().getInvalidLevels(containedLevels)) {
                    throw new LevelInvalidException(getMsg("exception.constraints.method_invalid_level",
                                                           invalidEffect.getName(),
                                                           methodSignature));
                }
            }
            for (IComponent paramRef : getInvalidParameterReferencesOfSet(constraints,
                                                                          signature,
                                                                          sootMethod.getParameterCount(),
                                                                          getParameterDimensions())) {
                throw new AnnotationInvalidException(getMsg("exception.constraints.invalid_param_ref",
                                                            methodSignature,
                                                            paramRef.toString()));
            }
            for (int i = 0; i < sootMethod.getParameterCount(); i++) {
                if (!containsSetParameterReferenceFor(constraints, signature, i)) {
                    throw new AnnotationInvalidException(getMsg("exception.constraints.no_param_ref",
                                                                methodSignature,
                                                                i));
                }
            }
            if (!containsSetProgramCounterReference(constraints) && !isClinit) {
                throw new AnnotationInvalidException(getMsg("exception.constraints.method_no_pc_ref",
                                                            methodSignature));
            }
            ...
            if (!getLevelMediator().checkParameterLevelsValidity(getListOfParameterLevels())) {
                for (ILevel level : getLevelMediator().getInvalidParameterLevels(getListOfParameterLevels())) {
                    throw new LevelInvalidException(getMsg("exception.level.parameter.invalid",
                                                           level.getName(),
                                                           methodSignature));
                }
            }
            if (isVoid) {
                if (returnLevel != null) {
                    throw new AnnotationInvalidException(getMsg("exception.level.return.void_method",
                                                                methodSignature));
                }
            } else {
                if (returnLevel == null) {
                    throw new AnnotationInvalidException(getMsg("exception.level.return.no_level",
                                                                methodSignature));
                } else {
                    if (!getLevelMediator().checkLevelValidity(returnLevel)) {
                        throw new LevelInvalidException(getMsg("exception.level.return.invalid",
                                                               returnLevel.getName(),
                                                               methodSignature));
                    }
                }
            ...
            if (!getLevelMediator().checkLevelsValidity(writeEffects)) {
                for (ILevel invalidEffect : getLevelMediator().getInvalidLevels(writeEffects)) {
                    throw new AnnotationInvalidException(getMsg("exception.effects.method_invalid",
                                                                invalidEffect.getName(),
                                                                methodSignature));
                }
            }
            if (!getLevelMediator().checkLevelsValidity(classWriteEffects)) {
                for (ILevel invalidEffect : getLevelMediator().getInvalidLevels(classWriteEffects)) {
                    throw new AnnotationInvalidException(getMsg("exception.effects.method_class_invalid",
                                                                invalidEffect.getName(),
                                                                methodSignature));
                }
            }
