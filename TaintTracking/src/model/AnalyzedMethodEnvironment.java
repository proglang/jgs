 ...
 * 
 * The {@link AnalyzedMethodEnvironment} provides a environment for analyzing a {@link SootMethod}. Therefore it extends the analysis method
...
public class AnalyzedMethodEnvironment extends MethodEnvironment {
	 ...
	 * {@link AnalyzedMethodEnvironment#effectsStore}. The created effect is caused by an assignment to an array. Therefore, the method
	 * requires the <em>security level</em> that is effected by this <em>write effect</em>, the source line number at which the effect occurs
	 ...
	 */
	public void addWriteEffectCausedByArrayAssign(ILevel effected, long srcLn, ArrayRef arrayRef) {
		effectsStore.addWriteEffect(new Effect(effected, srcLn, new ArrayAssignCause(arrayRef)));
	}

	/**
	 * The method creates a new <em>write effect</em> and adds this to the calculated effect store
	 * {@link AnalyzedMethodEnvironment#effectsStore}. The created effect is caused by an assignment to a field. Therefore, the method
	 * requires the <em>security level</em> that is effected by this <em>write effect</em>, the source line number at which the effect occurs
	 ...
	 */
	public void addWriteEffectCausedByAssign(ILevel effected, long srcLn, SootField sootField) {
		effectsStore.addWriteEffect(new Effect(effected, srcLn, new AssignCause(sootField)));
	}

	/**
	 * The method creates a new <em>write effect</em> and adds this to the calculated effect store
	 * {@link AnalyzedMethodEnvironment#effectsStore}. The created effect is caused by an inheritance of a class. Therefore, the method
	 * requires the <em>security level</em> that is effected by this <em>write effect</em>, the source line number at which the effect occurs
	 ...
	 */
	public void addWriteEffectCausedByClass(ILevel effected, long srcLn, SootClass sootClass) {
		effectsStore.addWriteEffect(new Effect(effected, srcLn, new ClassCause(sootClass)));
	}

	/**
	 * The method creates a new <em>write effect</em> and adds this to the calculated effect store
	 * {@link AnalyzedMethodEnvironment#effectsStore}. The created effect is caused by an invocation of a method that provides this effect.
	 * Therefore, the method requires the <em>security level</em> that is effected by this <em>write effect</em>, the source line number at
	 * which the effect occurs and also the {@link SootMethod} that is invoked and that provides this effect.
	 * 
	 * @param effected
	 *          Affected <em>security level</em> of the <em>write effect</em>.
	 * @param srcLn
	 ...
	 */
	public void addWriteEffectCausedByMethodInvocation(ILevel effected, long srcLn, SootMethod sootMethod) {
		effectsStore.addWriteEffect(new Effect(effected, srcLn, new MethodCause(sootMethod)));
	}

	/**
	 * Checks whether the calculated effects match the expected <em>write effects</em>, i.e. if a effect was calculated by the analysis, but
	 * the annotation didn't expected this effect, then this method will be logged with the logger. Also if the expected effects contains an
	 * effect which wasn't calculated then a warning will be logged, because of a superfluous effect. If the analyzed method is a static
	 * initializer method, the class <em>write effects</em> will be used as expected <em>write effects</em>, otherwise the effects of the
	...
	/**
	 * Method that returns the source line number of the currently handled statement. Note, that immediately after the initialization of this
	 * environment and before the start of flow analysis, the method will return {@code 0}.
	 * 
	 * @return The source line number of the current handled statement or if this wasn't set {@code 0}.
	 */
	...
	/**
	 * Method that returns the current handled {@link Stmt} of the analyzed method. Note, that immediately after the initialization of this
	 * environment and before the start of flow analysis, the method will return {@code null}.
	 * 
	...
	/**
	 * Logs the given message as a <em>side effect</em> together with the given source line number. The file name is created by the analyzed
	...
	/**
	 * Sets the current handled {@link Stmt} of the analyzed method. This handled statement will be used for logging. At the same time the
	 * method will also determine the source line number of the currently handled statement.
	 * 
	 * @param stmt
