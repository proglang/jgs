 ...
 * 
 * The {@link UpdateSwitch} extends the switch {@link TaintTrackingSwitch} and
 * contains the <em>security level</em> to which the level of the value should
 * be updated, the incoming and outgoing locals map of a specific state in the
 ...
 * possible value a method that updates the <em>security level</em> if it is
 ...
 */
public class UpdateSwitch extends TaintTrackingSwitch implements
		JimpleValueSwitch {
	...
	/**
	 * Method parameter information for which should be checked if the local
	 * variable that should be updated corresponds to this parameter
	 * information. This special case occurs only for the update of local
	 * variables (see {@link UpdateSwitch#caseLocal(Local)}).
	 */
	private MethodParameter methodParameter = null;
	/**
	 * Parameter reference for which should be checked if the local variable
	 * that should be updated corresponds to this parameter reference. This
	 * special case occurs only for the update of local variables (see
	 * {@link UpdateSwitch#caseLocal(Local)}).
	 */
	...
	/**
	 * Constructor of a {@link UpdateSwitch} that requires the current incoming
	 * and outgoing map of local variables as well as the environment of the
	 * current analyzed method. Also the <em>security level</em> to which the
	 * level of the applied value should be updated. Note, there is a special
	...
	public void caseAddExpr(AddExpr v) {
		throw new InvalidSwitchException(
				SecurityMessages.noLevelUpdatePossible("expression",
						v.toString(), getSrcLn()));
	}
	...
	public void caseArrayRef(ArrayRef v) {
		...
		if (!isEqualLevel(rightLevel, getWeakestSecurityLevel())) {
			...
			logSecurity(SecurityMessages.onlyLowArrayElements(
					getMethodSignature(), getSrcLn(), rightLevel,
					getWeakestSecurityLevel()));
		}
		if (!isWeakerOrEqualLevel(indexLevel, arrayLevel)) {
			logSecurity(SecurityMessages.assignmentToArrayWithStrongerIndex(
					getMethodSignature(), getSrcLn(), arrayLevel, indexLevel));
		}
		if (!isWeakerOrEqualLevel(rightLevel, arrayLevel)) {
			logSecurity(SecurityMessages.assignmentToWeakerArray(
					getMethodSignature(), getSrcLn(), arrayLevel, rightLevel));
		}
	}
	...
	public void caseCaughtExceptionRef(CaughtExceptionRef v) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase(
						"caught exception ref", "SecurityLevelUpdateSwitch",
						v.toString(), getSrcLn()));
	}
	...
	public void caseClassConstant(ClassConstant v) {
		throw new InvalidSwitchException(
				SecurityMessages.noLevelUpdatePossible("constant",
						v.toString(), getSrcLn()));
	}
	...
	public void caseDynamicInvokeExpr(DynamicInvokeExpr v) {
		throw new InvalidSwitchException(
				SecurityMessages.noLevelUpdatePossible("invocation",
						v.toString(), getSrcLn()));
	}
	 ...
	 * in the same way (see {@link UpdateSwitch#handleFieldRef(FieldRef)}).
	 * 
	 * @param v
	 ...
	 * given in the {@link UpdateSwitch} instance (see
	 * {@link UpdateSwitch#setParameterInformation(ParameterRef, MethodParameter)}
	...
	public void caseLocal(Local l) {
		...
		if (isWeakerLevel(valueLevel, localLevel)) {
			logWarning(SecurityMessages.weakenLocalVariable(
					getMethodSignature(), getSrcLn(), l.getName(), localLevel,
					valueLevel));
		}
	...
	/**
	 * If {@link UpdateSwitch#value} is set and the {@link UpdateSwitch#level}
	 * is {@code null}, then the <em>security level</em> of the contained value
	 * will be updated to the level of the given parameter level. Also it will
	 * be checked whether the handle assignment is valid, i.e. whether the name
	 * and the type of the local variable that is assigned corresponds to the
	 * name and type of the parameter.
	 * 
	 * @param v
	 *            Parameter reference that is assigned to the contained value
	 *            {@link UpdateSwitch#value} for which the
	 ...
	 * @throws UnimplementedSwitchException
	 *             If {@link UpdateSwitch#value} is not set and the
	 *             {@link UpdateSwitch#level} is not {@code null}, because the
	 *             contained value requires information of the given parameter
	 *             reference for updating the <em>security level</em>.
	 */
	@Override
		...
		if (level == null && value != null) {
			...
			throw new UnimplementedSwitchException(
					SecurityMessages.unimplementedSwitchCase("parameter ref",
							"SecurityLevelUpdateSwitch", v.toString(),
							getSrcLn()));
		}
	}
	 ...
	 * the same way (see {@link UpdateSwitch#handleFieldRef(FieldRef)}).
	 * 
	 * @param v
	 ...
	 * If {@link UpdateSwitch#value} is set and the {@link UpdateSwitch#level}
	 * is {@code null}, then the <em>security level</em> of the contained value
	 * will be updated to the weakest available <em>security level</em> because
	 * {@code this} has always the weakest <em>security level</em>.
	 * 
	 * @param v
	 ...
	 *             If {@link UpdateSwitch#value} is not set and the
	 *             {@link UpdateSwitch#level} is not {@code null}, because the
	 *             contained value requires information of the given this
	 *             reference for updating the <em>security level</em>.
	 */
	@Override
	...
	public void defaultCase(Object object) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase("default",
						"SecurityLevelUpdateSwitch", object.toString(),
						getSrcLn()));
	}
	...
	/**
	 * Sets the information of the method parameter, including the
	 * <em>security level</em>, which are required to update the level of the
	 * contained value.
	 * 
	 * @param parameterRef
	 *            The parameter reference that points to the parameter which is
	 *            assigned to a local variable.
	 * @param methodParameter
	 *            The method parameter information that corresponds to the given
	 *            parameter reference and that contains the
	 *            <em>security level</em> of the parameter to which the level of
	 *            the contained value should be updated.
	 */
	public void setParameterInformation(ParameterRef parameterRef,
