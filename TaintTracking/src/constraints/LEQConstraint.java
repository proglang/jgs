...
public final class LEQConstraint {
    ...
    public final boolean containsComponentInclBase(IComponent component) {
        if (isArrayReference(lhs)) {
            ...
            if (car.getBase().equals(component))
                return true;
        }
    ...
    public final boolean containsReturnReferenceFor(String signature) {
        return isReturnReference(lhs, signature)
               || isReturnReference(rhs, signature);
    }
    ...
    public final boolean containsReturnReferenceInclBaseFor(String signature) {
        if (isArrayReference(lhs)) {
            ComponentArrayRef car = (ComponentArrayRef) lhs;
            if (isReturnReference(car.getBase(), signature))
                return true;
        }
        if (isArrayReference(rhs)) {
            ComponentArrayRef car = (ComponentArrayRef) rhs;
    ...
    public final Set<ILevel> getContainedLevel() {
        ...
        if (isLevel(lhs))
            levels.add((ILevel) lhs);
        if (isLevel(rhs))
            levels.add((ILevel) rhs);
        return levels;
    }

    public final boolean containsParameterReferenceFor(String signature,
                                                       int position) {
        return isParameterReference(lhs, signature, position)
               || isParameterReference(rhs, signature, position);
    }

    public final boolean containsParameterReferenceFor(String signature) {
        return isParameterReference(lhs, signature)
               || isParameterReference(rhs, signature);
    }

    public final boolean containsParameterReferenceInclBaseFor(String signature) {
        if (isArrayReference(lhs)) {
            ComponentArrayRef car = (ComponentArrayRef) lhs;
            if (isParameterReference(car.getBase(), signature))
                return true;
        }
        if (isArrayReference(rhs)) {
            ComponentArrayRef car = (ComponentArrayRef) rhs;
    ...
    public boolean containsGeneratedLocal() {
        if (isLocal(lhs)) {
            if (((ComponentLocal) lhs).isGeneratedLocal())
                return true;
        }
        if (isLocal(rhs)) {
            if (((ComponentLocal) rhs).isGeneratedLocal())
                return true;
        }
        if (isArrayReference(lhs)) {
            ComponentArrayRef car = (ComponentArrayRef) lhs;
    ...
    public final Set<IComponent> getInvalidParameterReferencesFor(String signature,
                                                                  int count,
        ...
        if (isParameterReference(lhs)) {
            ...
            if (!paramRef.getSignature().equals(signature)
                || paramRef.getParameterPos() >= count) {
            ...
            if (isParameterReference(arrayRef.getBase())) {
                ComponentParameterRef paramRef =
                    (ComponentParameterRef) arrayRef.getBase();
                if (!paramRef.getSignature().equals(signature)
                    || paramRef.getParameterPos() >= count) {
    ...
    public final Set<IComponent> getInvalidReturnReferencesFor(String signature,
                                                               int dimension) {
        Set<IComponent> invalid = new HashSet<IComponent>();
        if (isReturnReference(lhs)) {
            ...
            if (isReturnReference(arrayRef.getBase())) {
                ComponentReturnRef returnRef =
                    (ComponentReturnRef) arrayRef.getBase();
                if (!returnRef.getSignature().equals(signature)) {
                    invalid.add(arrayRef);
                } else {
    ...
    public final boolean containsProgramCounterReferenceFor(String signature) {
        return isProgramCounterReference(lhs, signature)
               || isProgramCounterReference(rhs, signature);
    }
    ...
    public final LEQConstraint changeAllComponentsSignature(String signature) {
        return new LEQConstraint(getLhs().changeSignature(signature),
                                 getRhs().changeSignature(signature));
    }
