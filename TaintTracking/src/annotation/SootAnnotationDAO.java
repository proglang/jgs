...
public class SootAnnotationDAO extends AAnnotationDAO {
	...
	public SootAnnotationDAO(Class<? extends Annotation> annotationClass, AnnotationTag annotation) {
		...
		if (hasAnnotationArray(name)) {
			try {
				...
				for (int i = 0; i < aae.getNumValues(); i++) {
					list.add(convertAnnotation(aae.getValueAt(i), annotationClass));
				}
				return list;
	...
	public boolean hasAnnotation(String name) {
		return containsElement(name, ExtendedJNI.EXT_JNI_ANNOTATION);
	}

	public boolean hasAnnotationArray(String name) {
		return containsArray(name, ExtendedJNI.EXT_JNI_ANNOTATION);
	}

	public boolean hasBoolean(String name) {
		return containsElement(name, ExtendedJNI.JNI_BOOLEAN);
	}

	public boolean hasBooleanArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_BOOLEAN);
	}

	public boolean hasByte(String name) {
		return containsElement(name, ExtendedJNI.JNI_BYTE);
	}

	public boolean hasByteArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_BYTE);
	}

	public boolean hasChar(String name) {
		return containsElement(name, ExtendedJNI.JNI_CHAR);
	}

	public boolean hasCharArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_CHAR);
	}

	public boolean hasClass(String name) {
		return containsElement(name, ExtendedJNI.EXT_JNI_CLASS);
	}

	public boolean hasClassArray(String name) {
		return containsArray(name, ExtendedJNI.EXT_JNI_CLASS);
	}

	public boolean hasDouble(String name) {
		return containsElement(name, ExtendedJNI.JNI_DOUBLE);
	}

	public boolean hasDoubleArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_DOUBLE);
	}

	public boolean hasEnum(String name) {
		return containsElement(name, ExtendedJNI.JNI_ENUM);
	}

	public boolean hasEnumArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_ENUM);
	}

	public boolean hasFloat(String name) {
		return containsElement(name, ExtendedJNI.JNI_FLOAT);
	}

	public boolean hasFloatArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_FLOAT);
	}

	public boolean hasInt(String name) {
		return containsElement(name, ExtendedJNI.JNI_INT);
	}

	public boolean hasIntArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_INT);
	}

	public boolean hasLong(String name) {
		return containsElement(name, ExtendedJNI.JNI_LONG);
	}

	public boolean hasLongArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_LONG);
	}

	public boolean hasShort(String name) {
		return containsElement(name, ExtendedJNI.JNI_SHORT);
	}

	public boolean hasShortArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_SHORT);
	}

	public boolean hasString(String name) {
		return containsElement(name, ExtendedJNI.JNI_STRING);
	}

	public boolean hasStringArray(String name) {
		return containsArray(name, ExtendedJNI.JNI_STRING);
	}

	private boolean containsArray(String name, char kind) {
		if (containsElement(name, ExtendedJNI.JNI_ARRAY)) {
			try {
				AnnotationArrayElem aae = getArrayElem(name);
				return aae.getNumValues() == 0 || aae.getValueAt(0).getKind() == kind;
			} catch (ClassCastException e) {
				return false;
			}
		}
	...
	private boolean containsElement(String name, char kind) {
		return elements.containsKey(name) && elements.get(name).getKind() == kind;
	}

	private SootAnnotationDAO convertAnnotation(AnnotationElem ae, Class<? extends Annotation> annotationClass) throws NoSuchElementException {
		AnnotationAnnotationElem aae = (AnnotationAnnotationElem) ae;
		AnnotationTag at = aae.getValue();
		if (at.getType().equals(AnalysisUtils.getJNISignature(annotationClass))) {
			return new SootAnnotationDAO(annotationClass, at);
		}
		throw new NoSuchElementException(getMsg("annotation.invalid_annotation", annotationClass.getName()));
	}
	...
	private Class<?> convertClass(AnnotationElem ae) throws NoSuchElementException, ClassCastException {
	...
	private <V extends Enum<V>> V convertEnum(AnnotationElem ae, Class<V> enumClass) throws NoSuchElementException, ClassCastException {
		AnnotationEnumElem aee = (AnnotationEnumElem) ae;
		if (AnalysisUtils.getJNISignature(enumClass).equals(aee.getTypeName())) {
			return Enum.valueOf(enumClass, aee.getConstantName());
		}
		throw new NoSuchElementException(getMsg("annotation.invalid_enum", enumClass.getName()));
	}
	...
	private Class<?> getUsedClass(String jni) throws NoSuchElementException {
		if (PRIM_CLASSES.containsKey(jni)) {
			...
			String javaPath = AnalysisUtils.jniSignatureToJavaPath(jni);
			try {
				return Class.forName(javaPath);
			} catch (ClassNotFoundException e) {
				throw new NoSuchElementException(getMsg("annotation.not_found_class", javaPath));
			}
		}
	}
