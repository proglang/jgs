...
public class JavaAnnotationDAO extends AAnnotationDAO {
	...
	public boolean hasAnnotationArray(String name) {
		if (elements.containsKey(name)) {
			Method method = elements.get(name);
			return method.getReturnType().isArray() && method.getReturnType().getComponentType().isAnnotation();
		}
		return false;
	}
	...
	public boolean hasEnumArray(String name) {
		if (elements.containsKey(name)) {
			Method method = elements.get(name);
			return method.getReturnType().isArray() && method.getReturnType().getComponentType().isEnum();
		}
		return false;
	}
	...
	private <T> T getValue(String name, Class<T> type) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		Method method = elements.get(name);
