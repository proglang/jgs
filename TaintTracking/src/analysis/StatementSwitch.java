 ...
 * 
 * The {@link StatementSwitch} extends the switch {@link TaintTrackingSwitch}
 * and contains incoming and outgoing locals map of a specific state in the
 ...
 * possible statement a method that progress the statement, i.e. calculates or
 * updates the <em>security levels</em> of the statement components with the
 * help of other switches. Also the methods check for security violations, such
 * as an invalid return <em>security level</em> of a method, etc. This switch is
 * also responsible for checking whether a implicit flow starts.<br />
...
public class StatementSwitch extends TaintTrackingSwitch implements StmtSwitch {
	...
	/**
	 * Constructor of a {@link StatementSwitch} that requires the current
	 ...
	 */
	public StatementSwitch(AnalyzedMethodEnvironment analyzedMethodEnvironment,
	 ...
	 * {@link AssignStmt}. Therefore the method looks up the
	 * <em>security level</em> of the right hand side value and tries to update
	...
	public void caseBreakpointStmt(BreakpointStmt stmt) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase("break point stmt",
						"StatementSwitch", stmt.toString(), getSrcLn()));
	}

	/**
	 * Method, which should process the given statement of type
	 * {@link EnterMonitorStmt}, but is not implemented in the current version
	...
	public void caseEnterMonitorStmt(EnterMonitorStmt stmt) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase("enter monitor stmt",
						"StatementSwitch", stmt.toString(), getSrcLn()));
	}
	...
	public void caseExitMonitorStmt(ExitMonitorStmt stmt) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase("exit monitor stmt",
						"StatementSwitch", stmt.toString(), getSrcLn()));
	}
	...
	/**
	 * Method, which should process the given statement of type {@link IfStmt}.
	 * Therefore the method looks up which <em>security level</em> the condition
	 * of the if statement has and adds the {@link IfStmt} as well as the
	 * <em>security level</em> of the condition to the program counter, i.e. a
	 * new implicit flow will be started.
	 * 
	 * @param stmt
	 *            Statement that should be processed to check for security
	 *            violations.
	...
	/**
	 * Method, which should process the given statement of type
	 * {@link InvokeStmt}. Therefore the method processes the invoke expression
	 * without calculating the return <em>security level</em> of the invoked
	 * method. I.e. the method checks the <em>security levels</em> of the method
	 ...
	 * @see TaintTrackingSwitch#calculateInvokeExprLevel(soot.jimple.InvokeExpr,
	 *      boolean)
	 */
	@Override
	...
	public void caseLookupSwitchStmt(LookupSwitchStmt stmt) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase("lookup switch stmt",
						"StatementSwitch", stmt.toString(), getSrcLn()));
	}
	...
	public void caseRetStmt(RetStmt stmt) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase("ret stmt",
						"StatementSwitch", stmt.toString(), getSrcLn()));
	}
	...
	public void caseReturnStmt(ReturnStmt stmt) {
		...
		if (expectedLevel.equals(LevelMediator.VOID_LEVEL)) {
			logSecurity(SecurityMessages.voidReturnExpected(getFileName(),
					getSrcLn(), level, LevelMediator.VOID_LEVEL));
		} else {
			if (!isWeakerOrEqualLevel(level, expectedLevel)) {
				logSecurity(SecurityMessages.weakerReturnExpected(
						getMethodSignature(), getSrcLn(), level, expectedLevel));
			}
		}
	}
	...
	/**
	 * Method, which should process the given statement of type
	 * {@link ReturnVoidStmt}. Therefore the method checks whether the analyzed
	 * method is a 'void' return <em>security level</em>, if not then an error
	 * will be logged.
	 * 
	 * @param stmt
	 *            Statement that should be processed to check for security
	...
	public void caseTableSwitchStmt(TableSwitchStmt stmt) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase("table switch stmt",
						"StatementSwitch", stmt.toString(), getSrcLn()));
	}

	/**
	 * Method, which should process the given statement of type
	 * {@link ThrowStmt}, but is not implemented in the current version of this
	...
	public void caseThrowStmt(ThrowStmt stmt) {
		throw new UnimplementedSwitchException(
				SecurityMessages.unimplementedSwitchCase("throw stmt",
						"StatementSwitch", stmt.toString(), getSrcLn()));
	}

	/**
	 * Default case: Method, which should process the given object, but an
	 * object can't be processed to check for security violations. If method
	 * will be called an exception is thrown.
	 * 
	 * @param obj
