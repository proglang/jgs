 ...
 * 
 * The {@link TaintTrackingSwitch} is the base class for switches which are used
 * for the {@link TaintTracking} analysis. The base switch contains the incoming
 * and outgoing locals map of a specific
 * {@link TaintTracking#flowThrough(LocalsMap, soot.Unit, LocalsMap)} state as
 * well as the environment of the current analyzed method. The class provides
 * multiple methods which allows to log, to compare <em>security level</em>, to
 * check <em>write effect</em> and also to initialize the lookup and the update
 * of <em>security level</em>. <br />
 * The class {@link TaintTrackingSwitch} is the parent class of the following:
 * <ul>
 * <li>{@link StatementSwitch}: switch that processes statements - depending on
 * the statement the <em>security levels</em> will be calculated or updated
 * (with the help of other switches) for components of the statements and also
 * the statements will be checked for security violations.</li>
 * <li>{@link LookupSwitch}: switch that processes values - depending on the
 * value, the <em>security level</em> will be looked up for the value and also
 * stored in the switch. If necessary the switch checks for security violations,
 * too (e.g. for method invocations the parameter <em>security level</em>).</li>
 * <li>{@link UpdateSwitch}: switch that processes values - depending on the
 * value, the <em>security level</em> of this value will be updated by the given
 * level. The switch checks for security violations, too (e.g. occurring
 * <em>write effects</em> or invalid assignments).</li>
 * </ul>
 * 
 * <hr />
 * 
...
abstract public class TaintTrackingSwitch {
	...
	/**
	 * Constructor of a {@link TaintTrackingSwitch} that requires the current
	 ...
	 *            Current outgoing map of the local variables.
	 */
	public TaintTrackingSwitch(AnalyzedMethodEnvironment methodEnvironment,
	 ...
	 * analyzed {@link SootMethod}, which the environment stores, the source
	 * line number is the line number of current handled statement.
	 * 
	 * @param msg
	...
	/**
	 * Logs the given message as well as the {@link Exception} as an exception.
	 * The file name is created by the analyzed {@link SootMethod}, which the
	 * environment stores, the source line number is the line number of current
	 * handled statement.
	 * 
	 * @param msg
	...
	/**
	 * Logs the given message as a security violation. The file name is created
	 * by the analyzed {@link SootMethod}, which the environment stores, the
	...
	private void checkEffect(String effect) {
		if (out.hasProgramCounterLevel()) {
			...
			try {
				...
				logException(SecurityMessages.invalidLevelsComparisonInMap(
						getMethodSignature(), getSrcLn()), e);
			}
			if (isWeakerLevel(effect, pcLevel)) {
				analyzedMethodEnvironment.logEffect(getSrcLn(),
						SecurityMessages.effectWeakerThanPC(
								getMethodSignature(), getSrcLn(), effect,
								pcLevel));
			}
		}
	}
	 ...
	 * the given {@link UpdateSwitch} stores. In order to identify possible
	 * errors, also the unit, which contains the value, has to be given as a
	 * String. Occurring exceptions are logged.
	 * 
	 * @param value
	 *            Value for which the <em>security level</em> should be updated
	 *            to the given level.
	 * @param updateSwitch
	 *            {@link UpdateSwitch} which should be applied to the given
	 *            value and which contains the <em>security level</em> to which
	 *            the level of the value should be updated.
	 * @param containing
	 *            Unit that contains the given value as String.
	 * @see UpdateSwitch
	 */
	private void executeUpdateLevel(Value value, UpdateSwitch updateSwitch,
			String containing) {
		try {
			value.apply(updateSwitch);
		} catch (SwitchException e) {
			logException(SecurityMessages.catchSwitchException(
					getMethodSignature(), getSrcLn(), containing), e);
		}
	}
	 ...
	 *      long, ArrayRef)
	 */
	protected void addWriteEffectCausedByArrayAssign(String effect,
			ArrayRef arrayRef) {
		analyzedMethodEnvironment.addWriteEffectCausedByArrayAssign(effect,
				getSrcLn(), arrayRef);
		checkEffect(effect);
	}
	 ...
	 *      SootField)
	 */
	protected void addWriteEffectCausedByAssign(String effect,
			SootField sootField) {
		analyzedMethodEnvironment.addWriteEffectCausedByAssign(effect,
				getSrcLn(), sootField);
		checkEffect(effect);
	}
	 ...
	 *      SootClass)
	 */
	protected void addWriteEffectCausedByClass(String effect,
			SootClass sootClass) {
		analyzedMethodEnvironment.addWriteEffectCausedByClass(effect,
				getSrcLn(), sootClass);
		checkEffect(effect);
	}
	 ...
	 *      long, SootMethod)
	 */
	protected void addWriteEffectCausedByMethodInvocation(String effect,
			SootMethod sootMethod) {
		analyzedMethodEnvironment.addWriteEffectCausedByMethodInvocation(
				effect, getSrcLn(), sootMethod);
		checkEffect(effect);
	}
		...
		try {
			...
			if (invokeExpr.getArgCount() == invokedMethodParameter.size()) {
				...
				if (assignment) {
					...
					level = LevelMediator.VOID_LEVEL;
				}
			} else {
	...
	protected String calculateLevel(Value value, String containing) {
		LookupSwitch lookupSwitch = new LookupSwitch(analyzedMethodEnvironment,
				store, in, out);
		String level = getWeakestSecurityLevel();
		try {
		...
		try {
			...
			logException(SecurityMessages.noSecurityLevel(getSrcLn(),
					getMethodSignature(), value.toString()), e);
		}
		return level;
	}
	...
	/**
	 * Returns the file name of the class which declares the analyzed method.
	 * The resulting file name can differ from the real file name, e.g. for
	 * nested classes (see {@link SootUtils#generateFileName(SootClass)}) and
	 * the file name do not contain the file suffix.
	 * 
	 * @return The file name of the class which declares the analyzed method.
	 */
	protected String getFileName() {
		return SootUtils.generateFileName(analyzedMethodEnvironment
				.getSootMethod());
	}

	/**
	 * Returns the {@link SecurityLogger} of the
	 * {@link AnalyzedMethodEnvironment#mediator} object.
	 * 
	 * @return The logger.
	 */
	 ...
	 * {@link LevelMediator#VOID_LEVEL}), the method will log this
	 * exception and will return the weakest available <em>security level</em>
	 * (see {@link TaintTrackingSwitch#getWeakestSecurityLevel()}).
	 * 
	 * @param levels
	 ...
	 *         returned. Otherwise the the weakest available
	 *         <em>security level</em> will be returned.
	 */
	protected String getMaxLevel(List<String> levels) {
		String level = getWeakestSecurityLevel();
		try {
			level = getLevelMediator().getMaxLevel(levels);
		} catch (InvalidLevelException e) {
			String[] levelArray = new String[levels.size()];
			logException(SecurityMessages.invalidLevelsComparison(
	 ...
	 * of the two given levels is 'void' ({@link LevelMediator#VOID_LEVEL}
	 * ), the resulting level is also 'void'. If one of the both given levels is
	 * an invalid <em>security level</em> the method will log this exception and
	 * will return the weakest available <em>security level</em> (see
	 ...
	 *         {@link LevelMediator#VOID_LEVEL}).
	 */
	protected String getMaxLevel(String level1, String level2) {
		String level = getWeakestSecurityLevel();
		try {
			level = getLevelMediator().getMaxLevel(level1, level2);
		} catch (InvalidLevelException e) {
			logException(SecurityMessages.invalidLevelsComparison(
					getMethodSignature(), getSrcLn(), level1, level2), e);
		}
		return level;
	}

	/**
	 * Returns the readable method signature of the analyzed method. The
	 * returned signature contains the visibility of the method, the package
	...
	protected String getMethodSignature() {
		return SootUtils.generateMethodSignature(
				analyzedMethodEnvironment.getSootMethod(),
				Configuration.METHOD_SIGNATURE_PRINT_PACKAGE,
				Configuration.METHOD_SIGNATURE_PRINT_TYPE,
				Configuration.METHOD_SIGNATURE_PRINT_VISIBILITY);
	}
	...
	protected String getMinLevel(String level1, String level2) {
		String level = getWeakestSecurityLevel();
		try {
			level = getLevelMediator().getMinLevel(level1, level2);
		} catch (InvalidLevelException e) {
			logException(SecurityMessages.invalidLevelsComparison(
					getMethodSignature(), getSrcLn(), level1, level2), e);
		}
	...
	/**
	 * Returns the {@link LevelMediator} that allows the handling of
	 * <em>security level</em>.
	 * 
	 * @return The security annotation object.
	 */
	protected LevelMediator getLevelMediator() {
	 ...
	 * statement of the analyzed method. Note, that immediately after the
	 * initialization of the environment and before the start of flow analysis,
	 * the method will return {@code 0}.
	 * 
	 * @return The source line number of the current handled statement of the
	 *         analyzed method or if this wasn't set {@code 0}.
	 */
	 ...
	 * {@link Environment#getLevelMediator()#getWeakestSecurityLevel()}).
	 * 
	...
	/**
	 * Checks whether the first given level is equals to the second given level.
	 * If one of the both given levels is an invalid <em>security level</em> the
	 * method will log this exception and will return {@code false}.
	 * 
	 * @param level1
	 *            Level for which should be checked whether it is equals to the
	 *            second given level.
	 * @param level2
	 *            Level for which should be checked whether it is equals to the
	 *            first given level.
	 * @return {@code true} if the first given level is equals to the second
	...
	/**
	 * Checks whether the first given level is weaker than the second given
	 * level. If one of the both given levels is an invalid
	 * <em>security level</em> the method will log this exception and will
	 * return {@code false}.
	 * 
	 * @param level1
	 *            Level for which should be checked whether it is weaker than
	 *            the second given level.
	 * @param level2
	 *            Level for which should be checked whether it is equals or
	 *            stronger than the first given level.
	...
	protected boolean isWeakerLevel(String level1, String level2) {
		return isWeakerOrEqualLevel(level1, level2)
				&& !isEqualLevel(level1, level2);
	}
	...
	protected boolean isWeakerOrEqualLevel(String level1, String level2) {
		...
		try {
			is1WeakerEquals2 = getLevelMediator().isWeakerOrEqualsThan(
					level1, level2);
		} catch (InvalidLevelException e) {
			logException(SecurityMessages.invalidLevelsComparison(
					getMethodSignature(), getSrcLn(), level1, level2), e);
	 ...
	 *            Unit that contains the given value as String.
	 */
	protected void updateIdentityLevel(Value rightSide, Value leftSide,
			String containing) {
		UpdateSwitch updateSwitch = new UpdateSwitch(analyzedMethodEnvironment,
				store, in, out, null);
	...
	/**
	 * Updates the <em>security level</em> of the given value to the level which
	 * is given by the {@link TaintTrackingSwitch#analyzedMethodEnvironment} and
	 ...
	 *            {@link TaintTrackingSwitch#analyzedMethodEnvironment}.
	 */
	protected void updateParameterLevel(Value value, ParameterRef parameterRef) {
		...
		UpdateSwitch updateSwitch = new UpdateSwitch(analyzedMethodEnvironment,
				store, in, out, methodParameter.getLevel());
