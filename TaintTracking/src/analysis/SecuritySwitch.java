 ...
 * 
 * The {@link SecuritySwitch} is the base class for switches which are used for
 * the {@link SecurityLevelAnalysis} analysis. The base switch contains the
...
abstract public class SecuritySwitch<U> {
    ...
    /**
     * Constructor of a {@link SecuritySwitch} that requires the current
     ...
     *            Current outgoing map of the local variables.
     */
    protected SecuritySwitch(AnalyzedMethodEnvironment methodEnvironment,
            UsedObjectStore store, U in, U out) {
     ...
     * nested classes (see {@link AnalysisUtils#generateFileName(SootClass)})
     * and the file name do not contain the file suffix.
     * 
    ...
    /**
     * Returns the {@link AnalysisLog} of the
     ...
     * the resulting level is also 'void'. If one of the both given levels is an
     * invalid <em>security level</em> the method will log this exception and
     ...
     * {@link LevelMediator#VOID_LEVEL}), the method will log this exception and
     * will return the weakest available <em>security level</em> (see
     * {@link SecuritySwitch#getWeakestSecurityLevel()}).
     * 
     * @param levels
