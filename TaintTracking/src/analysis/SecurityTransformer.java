...
import static util.AnalysisUtils.isMethodOfDefinition;
...
public class SecurityTransformer extends BodyTransformer {
     ...
     * @param type
     */
    public SecurityTransformer(ILevelMediator mediator, AnalysisLog log,
            AnnotationExtractor extractor, boolean instantLogging,
            AnalysisType type) {
        super();
    ...
    private void doAnalysis(SootMethod sootMethod, UnitGraph graph) {
        if (!isMethodOfDefinition(sootMethod)) {
            ...
            if (type.equals(CONSTRAINTS)) {
                SecurityConstraintsAnalysis sca =
                    new SecurityConstraintsAnalysis(log,
                                                    sootMethod,
                                                    mediator,
                                                    graph,
                                                    extractor.getUsedObjectStore());
                sca.checkAnalysis();
            } else if (type.equals(LEVELS)) {
                SecurityLevelAnalysis sla =
                    new SecurityLevelAnalysis(log,
                                              sootMethod,
                                              mediator,
                                              graph,
                                              extractor.getUsedObjectStore());
