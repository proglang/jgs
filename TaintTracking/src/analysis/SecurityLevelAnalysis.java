 ...
 * 
 * The {@link SecurityLevelAnalysis} is the security analysis and extends the
 ...
 * {@link SecurityLevelAnalysis#checkAnalysis()} has to be called.
 * 
 * <hr />
 * 
 * @author Thomas Vogel
...
public class SecurityLevelAnalysis extends ASecurityAnalysis<Unit, LocalsMap> {
    ...
    /**
     * Constructor of {@link SecurityLevelAnalysis} which checks automatically
     * the given graph of the also given method for security violations, i.e.
     ...
     * {@link SecurityLevelAnalysis#checkAnalysis()} has to be called for the
     ...
     * @param store
     */
    protected SecurityLevelAnalysis(AnalysisLog log, SootMethod sootMethod,
            ILevelMediator mediator, DirectedGraph<Unit> graph,
    ...
    private void checkEndOfImplicitFlow(Stmt statement,
                                        ...
                                        LocalsMap out) {
        List<IfStmt> ifStmts =
            new ArrayList<IfStmt>(in.getProgramCounter().keySet());
    ...
    protected LocalsMap entryInitialFlow() {
        LocalsMap map =
            new LocalsMap(getAnalyzedEnvironment().getSootMethod()
                                                  .getActiveBody()
                                                  .getLocals(),
                          getAnalyzedEnvironment().getLevelMediator());
        return map;
    }
    ...
    protected void flowThrough(LocalsMap in, Unit d, LocalsMap out) {
        ...
        try {
            SecurityLevelStmtSwitch stmtSwitch =
                new SecurityLevelStmtSwitch(getAnalyzedEnvironment(),
                                            getStore(),
                                            in,
                                            out);
            ...
            throw new AnalysisException(getMsg("exception.analysis.other.error_switch",
                                               stmt.toString(),
     ...
     * {@link SecurityLevelAnalysis#doAnalysis()} method.
     * 
     * @param in1
