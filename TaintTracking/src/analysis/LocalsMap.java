 ...
 * <h1>Locals map</h1>
 * 
 * The {@link LocalsMap} stores the {@link Local} of a specific analyzed method. The {@link LocalsMap} provides the possibility to map a
 * {@link Local} to a <em>security level</em>, i.e. the locals provide their corresponding <em>security level</em>. Also the
...
public class LocalsMap {
	...
	/**
	 * Map that maps a specific {@link Local} to a <em>security level</em>. This level is the <em>security level</em> of the local variable.
	 */
	private Map<Local, ILevel> localMap = new HashMap<Local, ILevel>();
	/**
	 * {@link LevelMediator} object that provides the possibility to handle and to compare <em>security levels</em>.
	 */
	private final ILevelMediator mediator;
	/**
	 * Map that maps a specific {@link IfStmt} to a <em>security level</em>. The level represents the <em>security level</em> of the
	 * condition.
	 */
	private Map<IfStmt, ILevel> programCounter = new HashMap<IfStmt, ILevel>();

	/**
	 * Constructor of a {@link LocalsMap} object that requires a {@link Chain} of locals, which are all used {@link Local} of the analyzed
	 * method. Also the constructor requires an {@link LevelMediator} object that provides the possibility to compare and to validate
	 * <em>security levels</em>.
	 * 
	 * @param chain
	 *          Chain of all used {@link Local} in the currently analyzed method.
	 * @param mediator
	 *          Object that provides the possibility to compare and to validate <em>security levels</em>.
	...
	public void addAllStronger(List<ExtendedLocal> locals) {
		for (ExtendedLocal extendedLocal : locals) {
			...
			if (localMap.containsKey(extendedLocal.getLocal())) {
				...
				if (existingLevel == null) existingLevel = mediator.getDefaultVariableSecurityLevel();
	...
	/**
	 * Adds a new <em>program counter</em> with the given {@link IfStmt} and the given <em>security level</em> of the condition to the
	 * <em>program counter</em> map. I.e. a new implicit flow will be started by adding those values to the map.
	 * 
	 * @param ifStmt
	...
	/**
	 * Clears the map of locals. After calling this method the {@link LocalsMap} doesn't store any {@link Local} together with its
	...
	/**
	 * Checks whether the given {@link Local} is contain by the {@link LocalsMap#localMap}, thus stores also a <em>security level</em> for
	 * this given local variable.
	 * 
	 * @param local
	 *          {@link Local} for which should be checked whether the store contains this local variable.
	 ...
	 * An other object is only equals to this if the other object is this object or if it is a {@link LocalsMap} and the map of {@link Local}
	 * as well as the map of <em>program counter</em> are equals.
	 * 
	 * @param obj
	 *          Object for which should be checked whether it is equals to this {@link LocalsMap}.
	 * @return {@code true} if the given object is a {@link LocalsMap} and if the map of {@link Local} as well as the map of
	...
	/**
	 * Returns the <em>security level</em> of the given {@link Local} by a lookup in the {@link LocalsMap#localMap}.
	 * 
	 * @param local
	...
	/**
	 * Returns the <em>program counter</em> map that contains for all stored {@link IfStmt} the <em>security level</em> of the if-condition.
	 * 
	...
	/**
	 * Returns the strongest <em>security level</em> of the <em>program counter</em> map, i.e. if the analysis checks a {@link Unit} inside of
	 * multiple implicit flows, the <em>security level</em> of the implicit flow with the strongest {@link IfStmt} condition will be returned.
	 * 
	...
	/**
	 * Checks whether the <em>program counter</em> contains a {@link IfStmt}. I.e. if the map doesn't contain an {@link IfStmt} then the
	 * analysis doesn't handle a implicit flow for the current checked {@link Unit}.
	 * 
	 * @return {@code true} if the <em>program counter</em> map contains an {@link IfStmt}, means the analysis checks an {@link Unit} inside
	...
	/**
	 * Removes the <em>program counter</em> with the given {@link IfStmt}, i.e. finishes the implicit flow which starts at the given
	 * {@link IfStmt}.
	 * 
	 * @param ifStmt
	 *          {@link IfStmt} which should be removed from the <em>program counter</em>, i.e. the {@link IfStmt} at which the implicit flow
	 *          starts that should be removed.
	 */
	public void removeProgramCounterLevel(IfStmt ifStmt) {
		if (!programCounter.isEmpty()) programCounter.remove(ifStmt);
	}

	/**
	 * Method that sets the <em>security level</em> of the given {@link Local} in the store to the given level.
	 * 
	 * @param local
