 ...
 * 
 * The {@link TaintTracking} is the security analysis and extends the
 ...
 * {@link TaintTracking#checkAnalysis()} has to be called.
 * 
 * <hr />
 * 
 * @author Thomas Vogel
...
public class TaintTracking extends ForwardFlowAnalysis<Unit, LocalsMap> {
	 ...
	 * 
	 * The {@link Dominator} class provides the post dominator as well as the
	 * dominator objects. These allow to distinguish the (post-) dominator set
	 * for a specific unit. The class provides a method a method that checks
	 * whether the post dominator set of a given if-statement contains the
	...
	protected static class Dominator {
		...
		/**
		 * Constructor of a {@link Dominator} which requires a graph in order to
		 * generated the (post-) dominator finder.
		 * 
		 * @param graph
		...
		/**
		 * Checks whether the post-dominator set of the given if-statement
		 * contains the given statement. I.e. the given statement is a
		 * post-dominator of the if-statement.
		 * 
		 * @param ifStmt
		 *            if-statement whose post-dominator set is generated.
		 * @param s
		 *            statement for which will be checked whether it is
		 *            contained by the post-dominator set.
		 * @return {@code true} if the post-dominator set of the given
		 *         if-statement contains the other given statement, otherwise
		 *         {@code false}.
		 */
	...
	/**
	 * Constructor of {@link TaintTracking} which checks automatically the given
	 * graph of the also given method for security violations, i.e. violations
	 * of <em>security levels</em> or violations of <em>write effects</em>. The
	 * constructor requires in addition a logger that allows the logging of
	 * exceptions and violations, and a security annotation instance that allows
	 * the handling of <em>security levels</em> for this method. Note, that the
	 * method {@link TaintTracking#checkAnalysis()} has to be called for the
	 * final check of the <em>write effects</em>.
	 * 
	 * @param log
	 *            The {@link SecurityLogger} which provides the logging of
	...
	public TaintTracking(SecurityLogger log, SootMethod sootMethod,
		...
		if (analyzedMethodEnvironment != null || !interrupt) {
			doAnalysis();
		} else {
			// TODO: Logging
			System.err.println("Can't create an analyzed method environment "
	...
	public void checkAnalysis() {
		if (analyzedMethodEnvironment != null)
	 ...
	 * {@link StatementSwitch} switch to the statement, i.e. calculates or
	 * updates the <em>security levels</em> of the statement components to check
	...
	public void flowThrough(LocalsMap in, Unit d, LocalsMap out) {
		...
		StatementSwitch stmtSwitch = new StatementSwitch(
				analyzedMethodEnvironment, store, in, out);
		try {
			stmt.apply(stmtSwitch);
		} catch (SwitchException e) {
			String fileName = SootUtils
					.generateFileName(analyzedMethodEnvironment.getSootMethod());
			...
			analyzedMethodEnvironment.getLog().exception(
					fileName,
					srcLn,
					SecurityMessages.catchSwitchException(methodSignature,
							srcLn, stmt.toString()), e);
		}
	}

	/**
	 * Checks whether the given statement is the end of an implicit flow. If
	 * this is the case, then the corresponding if statement which is the start
	 * of this implicit flow will be removed from the program counter in the
	 * outgoing locals map. Note, that the method will not change the incoming
	 * locals map.
	 * 
	 * @param statement
	 *            Statement for which should be checked whether it is the end of
	 *            an implicit flow.
	 * @param in
	 ...
	 */
	private void checkEndOfImplicitFlow(Stmt statement, LocalsMap in,
			LocalsMap out) {
		List<IfStmt> ifStmts = new ArrayList<IfStmt>(in.getProgramCounter()
				.keySet());
	...
	protected void merge(LocalsMap in1, LocalsMap in2, LocalsMap out) {
		...
		try {
			...
			analyzedMethodEnvironment.getLog().exception(
					fileName,
					srcLn,
					SecurityMessages.invalidLevelsComparisonInMap(
							methodSignature, srcLn), e);
		}
	}
