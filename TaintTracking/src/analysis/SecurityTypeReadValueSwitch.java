...
/**
 * <h1>Lookup switch for the {@link SecurityTypeAnalysis} analysis</h1>
 * 
 * The {@link SecurityTypeReadValueSwitch} extends the switch {@link SecurityTypeSwitch} and contains incoming and outgoing locals map of a
 * specific state in the progress of the method {@link SecurityTypeAnalysis#flowThrough(LocalsMap, soot.Unit, LocalsMap)} as well as the
 ...
 * {@link SecurityTypeReadValueSwitch#level}. <br / >
 * 
 ...
 */
public class SecurityTypeReadValueSwitch extends SecurityTypeSwitch implements JimpleValueSwitch {

	/** The calculated <em>security level</em> of the applied value. */
	private ILevel level = null;

	/**
	 * Constructor of a {@link SecurityTypeReadValueSwitch} that requires the current incoming and outgoing map of local variables as well as
	 ...
	 * {@link IdentityStmt} a {@link SecurityTypeWriteValueSwitch} where the level is set to {@code null} and the left hand side of the
	 * statement is added to the {@link SecurityTypeWriteValueSwitch} with the method
	 * {@link SecurityTypeWriteValueSwitch#setIdentityInformation(Value)}. If the identity statement is a assignment of a parameter reference
	 ...
	 */
	protected SecurityTypeReadValueSwitch(AnalyzedMethodEnvironment analyzedMethodEnvironment, UsedObjectStore store, LocalsMap in,
			LocalsMap out) {
		super(analyzedMethodEnvironment, store, in, out);
	}

	/**
	 * Looks up the <em>security level</em> for the given binary expression and stores the level in {@link SecurityTypeReadValueSwitch#level}.
	 ...
	 * @see SecurityTypeReadValueSwitch#handleBinaryOperation(BinopExpr)
	 */
	@Override
	...
	/**
	 * Looks up the <em>security level</em> for the given array reference and stores the level in {@link SecurityTypeReadValueSwitch#level}.
	...
	/**
	 * Looks up the <em>security level</em> for the given cast expression and stores the level in {@link SecurityTypeReadValueSwitch#level}.
	 * 
	 * @param v
	...
	/**
	 * The method should look up the <em>security level</em> of a {@link CaughtExceptionRef}, but it is not implemented how to look up the
	 * level of a caught exceptions reference.
	 * 
	 * @param v
	 *          The caught exception reference for which the <em>security level</em> should be looked up.
	...
	/**
	 * Looks up the <em>security level</em> for the given constant and stores the level in {@link SecurityTypeReadValueSwitch#level}. For a
	 ...
	 * level in {@link SecurityTypeReadValueSwitch#level}. Also the parameter <em>security level</em> and the <em>write effects</em> will be
	 * handled.
	 * 
	 * @param v
	 ...
	 * @see SecurityTypeReadValueSwitch#handleInvokeExpr(InvokeExpr)
	 */
	@Override
	 ...
	 * in {@link SecurityTypeReadValueSwitch#level}. Additionally, the base of the field will be checked and if the level of the base if
	 * stronger than the resulting <em>security level</em> of the field, then this base <em>security level</em> will be stored in
	 * {@link SecurityTypeReadValueSwitch#level}.
	 * 
	 * @param v
	 ...
	 * @see SecurityTypeReadValueSwitch#handleFieldAccess(FieldRef)
	 * @see SecurityTypeReadValueSwitch#handleBase(Value, Value)
	 */
	@Override
	 ...
	 * @see SecurityTypeReadValueSwitch#handleOneValue(Value, Value)
	 */
	@Override
	...
	/**
	 * Looks up the <em>security level</em> for the given length expression and stores the level in {@link SecurityTypeReadValueSwitch#level}.
	 ...
	 * @see SecurityTypeReadValueSwitch#handleUnaryOperation(UnopExpr)
	 */
	@Override
	...
	/**
	 * Looks up the <em>security level</em> for the given local variable and stores the level in {@link SecurityTypeReadValueSwitch#level}.
	...
	/**
	 * Looks up the <em>security level</em> for the given new expression and stores the level in {@link SecurityTypeReadValueSwitch#level}.
	 ...
	 * implemented in the {@link SecurityTypeWriteValueSwitch}.
	 * 
	 * @param v
	 ...
	 * in {@link SecurityTypeReadValueSwitch#level}.
	 * 
	 * @param v
	...
	/**
	 * Default case: the method should look up the <em>security level</em> of an {@link Object}, but a level for an object could not be looked
	 * up.
	 * 
	 * @param object
	...
	/**
	 * Method that handles the given value as a base and expects that the level {@link SecurityTypeReadValueSwitch#level} was calculated
	 ...
	 * 
	 * @param value
	 *          The value which is the base and for which the level should be calculated and checked whether it is stronger than the existing
	 *          level.
	 * @param containing
	 *          The value which encapsulates the base value for logging reasons.
	 */
	private void handleBase(Value value, Value containing) {
		ILevel baseLevel = calculateLevel(value, containing.toString());
	 ...
	 * in the variable {@link SecurityTypeReadValueSwitch#level}.
	 * 
	 * @param expr
	 ...
	 * and set this <em>security level</em> to the calculated level {@link SecurityTypeReadValueSwitch#level}. If the field is a library field
	 ...
	 * {@link SecurityTypeReadValueSwitch#level}. I.e. the method checks also the <em>security levels</em> of the method parameters. If an
	 ...
	 * @param invokeExpr
	 *          The invoke expression, for which the level should be looked up.
	 * @see SecurityTypeSwitch#calculateInvokeExprLevel(InvokeExpr, boolean)
	 */
	...
	/**
	 * Looks up the <em>security level</em> of the given value and stores the resulting level in {@link SecurityTypeReadValueSwitch#level}.
	...
	private void handleOneValue(Value value, Value containing) {
		ILevel valueLevel = calculateLevel(value, containing.toString());
		this.level = valueLevel;
	}

	/**
	 * Looks up the <em>security level</em> for the given unary expression and stores the level in {@link SecurityTypeReadValueSwitch#level}.
	 * 
	 * @param expr
