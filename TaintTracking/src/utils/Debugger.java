...
public class Debugger {
	...
	protected enum TextStyle implements Style {
		RESET(0), BOLD(1), ITALIC(3), UNDERLINE(4), NEGATIVE(7), CONCEAL(8), CROSSEDOUT(9), DOUBLE(21);
	...
	public static void show(Header header, String txt) {
		formatAndShow(header.header, header.subheader, new String[] { txt }, null);
	}

	public static void show(Header header, String txt, RuntimeException exception) {
		formatAndShow(header.header, header.subheader, new String[] { txt }, exception);
	}
	...
	public static void show(Header header, Collection<String> txts) {
		formatAndShow(header.header, header.subheader, txts.toArray(new String[] {}), null);
	}

	public static void show(Header header, Collection<String> txts, RuntimeException exception) {
		formatAndShow(header.header, header.subheader, txts.toArray(new String[] {}), exception);
	}
	...
	public static void show(Header header, String[] txts, RuntimeException exception) {
		formatAndShow(header.header, header.subheader, txts, exception);
	}

	public static void showDependingOnError(Header header, String[] txts, RuntimeException exception) {
		if (exception != null) formatAndShow(header.header, header.subheader, txts, exception);
	}

	private static void formatAndShow(String title, String position, String[] txts, RuntimeException exception) {
		if (DEBUG) {
	...
	private static boolean highIntensity() {
		...
		if (System.getenv("HIGH_INTENSITY") != null && System.getenv("HIGH_INTENSITY").equals("true")) {
			highIntensity = true;
		}
	...
	private static String reduceString(String string, int additional) {
		int size = Math.max(3, (Math.round(Configuration.DISPLAY_SIZE / 12) * 11) - additional);
