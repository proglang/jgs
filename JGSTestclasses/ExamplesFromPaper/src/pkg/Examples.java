...
public class Examples {
    ...
    static int maxMsg(Logger log, int x, int y) {
        if (x <= y) { x = y; }
        log.buf = SString.MAX_WAS_CALLED;
        return x;
    }
    ...
    static int maxMsgDyn(Logger log, int x, int y) {
        if (x <= y) { x = y; }
        log.dbuf = SString.MAX_WAS_CALLED;
        return x;
    }
    ...
    static void logResults(Logger log, boolean privMode, SString h, SString l) {
        log.dbuf = SString.append(SString.PUBLIC_RESULT, Casts.castLowToDyn(l));
        if (privMode) {
            log.dbuf = SString.append(SString.SECRET_RESULT, Casts.castHighToDyn(h));
        }
        if (!privMode) {
